...
import { BUDGET_QUERY_SCHEMA, DATE_QUERY_SCHEMA } from '@/types';
...
export const getBudget = async (req: Request, res: Response) => {
  ...
  const validation = BUDGET_QUERY_SCHEMA.safeParse(req.query);
  if (!validation.success) {
    throw new AppError('Validation failed', 400, validation.error.errors);
  }
  const { from, to, needs, wants, savings } = validation.data;
